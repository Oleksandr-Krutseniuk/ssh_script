...
source /home/*****/var_file.txt # файл зі змінними, які імпортуються
...
file_path="/home/*****/testfile" # файл для відправки
...
scp -i "$key" -P "$port_number" "$archive_name" "$backup_user@$backup_host:/home/*****/$archive_name" > /dev/null # якщо потрібно - 
...
received_hashsum=$(ssh -i "$key" -p "$port_number" $backup_user@$backup_host "sha256sum /home/*****/$archive_name" | awk '{print $1}')
